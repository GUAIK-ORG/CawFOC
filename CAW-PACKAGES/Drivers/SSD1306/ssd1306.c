 ...
 * @LastEditors: Rick
 * @LastEditTime: 2023-06-25 12:43:13
                                     ...
                                     uint16_t size)
{
  ...
  if (!G_SSD1306_Instance.initialized)
    return sta;
 ...
 * @return {*} 处理状态
 */
int SSD1306_Init(I2C_HandleTypeDef *hi2c, bool enable_dma)
{
  if (G_SSD1306_Instance.initialized)
    return -1;
...
int SSD1306_WriteChar(char c, uint8_t x, uint8_t y, SSD1306_FONT font)
{
  ...
  if (font == FONT_6x8)
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      ...
      {
        if (y / SSD1306_ROW_NUM + 1 >= SSD1306_PAGE_NUM)
          return -1;
        uint8_t page = y / SSD1306_ROW_NUM;
